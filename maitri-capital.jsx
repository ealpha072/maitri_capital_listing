  ...
  return (
    <>
      <nav style={{
        ...
        <button onClick={() => { onHome(); setMenuOpen(false); }} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: isTablet ? 18 : 21,
            color: scrolled || menuOpen ? T.slateDark : "white", letterSpacing: "0.12em", lineHeight: 1.2 }}>
            Maitri Capital
          </div>
...
const Hero = ({ onExplore }) => {
  ...
  >
    ...
    />
    <div 
      style={{ position: "relative", zIndex: 2, padding: isMobile ? "0 24px 60px" : isTablet ? "0 40px 70px" : "0 60px 90px", maxWidth: 900 }} 
      className="fadeUp-1"
    >
      ...
      <p 
        style={{ fontSize: isMobile ? 13 : 14, lineHeight: 1.9, color: "rgba(255,255,255,0.65)", maxWidth: 460, letterSpacing: "0.03em", marginBottom: isMobile ? 32 : 42 }}>
      ...
      <div style={{ display: "flex", gap: isMobile ? 16 : 24, alignItems: "center", flexWrap: "wrap" }}>
        <button onClick={onExplore} style={{ fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", background: T.gold, color: "white", border: "none", padding: isMobile ? "14px 28px" : "16px 40px", cursor: "pointer", transition: "background 0.3s" }}
...
const StatsBar = ({ listings }) => {
  ...
  return (
    <div style={{ background: T.slateDark, display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", borderBottom: `1px solid rgba(191,144,0,0.15)` }}>
      {stats.map((s, i) => (
        <div key={i} style={{ padding: isMobile ? "24px 16px" : "34px 20px", textAlign: "center",
  ...
  return (
    ...
    >
      <img 
        src={listing.image} 
        alt={listing.title} 
        style={{ width: "100%", height: featured ? (cols === 1 ? 320 : 520) : 300, 
        objectFit: "cover", 
        display: "block", 
        ...
        {sent ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 420 }}>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
              {[["First Name","James"],["Last Name","Whitmore"]].map(([l, p]) => (
                <div key={l}>
            ...
            {[["Email","james@familyoffice.com"],["Organisation","Vantage Family Office"],["Investment Appetite","e.g. $5M–$25M"]].map(([l, p]) => (
              <div key={l}>
                <div style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "rgba(255,255,255,0.32)", marginBottom: 6 }}>{l}</div>
                <input placeholder={p} style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "white", padding: "11px 14px", fontSize: 13, outline: "none" }}
                  onFocus={e => e.target.style.borderColor = T.gold}
    ...
    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : isTablet ? "1fr 1fr" : "2fr 1fr 1fr 1fr", gap: isMobile ? 36 : 40, marginBottom: 44, paddingBottom: 44, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      {[["Portfolio",["Safari Lodges","Coastal Resorts","Mountain Retreats","Desert Sanctuaries"]],
        ...
        <div key={title}>
          ...
          {links.map(l => <div key={l} style={{ fontSize: 13, color: "rgba(255,255,255,0.36)", marginBottom: 9, letterSpacing: "0.03em" }}>{l}</div>)}
        </div>
      ))}
    </div>
    ...
    <div style={{ background: T.cream, minHeight: "100vh" }}>
      ...
      <div style={{ background: T.slateDark, display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : `repeat(${[listing.investment, listing.irr || listing.targetIrr, listing.rooms, listing.opening].filter(Boolean).length}, 1fr)` }}>
        {[
          ...
          <div key={i} style={{ padding: isMobile ? "20px 16px" : "26px 24px", textAlign: "center",
      ...
      <div style={{ padding: isMobile ? "52px 20px" : isTablet ? "64px 32px" : "90px 60px", display: "grid", gridTemplateColumns: isTablet ? "1fr" : "1fr 360px", gap: isTablet ? 40 : 80, maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div>
          ...
          <div style={{ background: T.slateDark, padding: isMobile ? 24 : 36 }}>
            ...
            <button style={{ width: "100%", background: T.gold, border: "none", color: "white", padding: 15, fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", cursor: "pointer" }}
              onMouseEnter={e => e.currentTarget.style.background = "#a67c00"}
              onMouseLeave={e => e.currentTarget.style.background = T.gold}>
              Request Access
            </button>
          ...
          {listings.map((l, i) => (
            <div key={l.id}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "minmax(180px,2fr) minmax(80px,1fr) 110px" : "2.5fr 1.2fr 1fr 1fr 1fr 130px", padding: "14px 16px", alignItems: "center", borderBottom: i < listings.length - 1 ? `1px solid ${T.border}` : "none", background: i % 2 === 0 ? "white" : T.cream, minWidth: isMobile ? 400 : "auto", transition: "background 0.2s" }}
                ...
                <div>
                  <span style={{ fontSize: 9, letterSpacing: "0.15em", textTransform: "uppercase", padding: "4px 8px",
...
const PublicSite = ({ listings, onSelectListing, onAdmin }) => {
  ...
  return (
    <div>
      <Nav onAdmin={onAdmin} onHome={() => window.scrollTo({ top: 0, behavior: "smooth" })} adminMode={false} />
  ...
  if (view === "detail" && selectedListing) return (
    <>
      ...
      <Nav
        ...
        onHome={() => { setView("public"); window.scrollTo(0, 0); }}
        adminMode={false}
      />
      <ListingDetail
