...
const Footer = () => {
  ...
  return (
    <footer style={{ background: T.ink, padding: isMobile ? "48px 24px 28px" : isTablet ? "52px 32px 32px" : "60px 60px 36px" }}>
      ...
      >
        <div>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: "white", letterSpacing: "0.1em", marginBottom: 4 }}>
            Maitri Capital
          </div>
          <div style={{ fontSize: 9, letterSpacing: "0.35em", textTransform: "uppercase", color: T.gold, marginBottom: 14 }}>
            Hospitality Investment
          </div>
          <p style={{ fontSize: 13, lineHeight: 1.8, color: "rgba(255,255,255,0.32)", maxWidth: 240 }}>
        ...
        {footerLinks.map(({ title, links }) => (
          <div key={title}>
            <div style={{ fontSize: 9, letterSpacing: "0.35em", textTransform: "uppercase", color: T.gold, marginBottom: 18 }}>
              {title}
            </div>
            {links.map(({ label, href }) =>
              href ? (
                ...
                <div key={label} style={{ fontSize: 13, color: "rgba(255,255,255,0.36)", marginBottom: 9, letterSpacing: "0.03em" }}>
                  {label}
                </div>
              )
            )}
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: isMobile ? 16 : 24, flexWrap: "wrap" }}>
          {["Privacy Policy", "Terms of Use"].map((l) => (
            <span key={l} style={{ fontSize: 11, color: "rgba(255,255,255,0.18)", letterSpacing: "0.08em" }}>
              {l}
            </span>
          ))}
        </div>
